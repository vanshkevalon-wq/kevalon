...
function TestimonialCard({ r }) {
  return (
    <div
      className="relative bg-white border border-[rgba(97,187,197,0.16)] rounded-3xl p-8 pb-6 md:p-5 md:pb-4 overflow-hidden flex flex-col gap-0 transition-all duration-300 ease-[cubic-bezier(0.22,1,0.36,1)] hover:-translate-y-[7px] hover:shadow-[0_20px_52px_rgba(3,70,101,0.12),0_0_0_1px_rgba(97,187,197,0.12)] group"
    ...
    >
      ...
      {/* quote */}
      <div className="text-[2.2rem] md:text-[1.6rem] leading-none mb-4.5 md:mb-3 opacity-50" aria-hidden="true" style={{ background:`linear-gradient(135deg,${r.color},#034665)`,WebkitBackgroundClip:'text',WebkitTextFillColor:'transparent',backgroundClip:'text' }}>
        <i className="bi bi-quote" />
      </div>

      <p className="text-[0.92rem] md:text-[0.82rem] text-[#2d4f68] leading-[1.85] md:leading-[1.7] italic m-0 mb-5 md:mb-3 flex-1">"{r.text}"</p>

      {/* metric */}
      <div className="inline-flex items-baseline gap-2 border border-[rgba(97,187,197,0.18)] rounded-[12px] px-4 py-1 md:px-3 md:py-0.5 mb-5 md:mb-3 self-start" style={{ background:'linear-gradient(135deg,rgba(97,187,197,0.07) 0%,rgba(3,70,101,0.04) 100%)' }}>
        <span className="text-[1.4rem] md:text-[1.1rem] font-[900] leading-none" style={{ background:`linear-gradient(120deg,${r.color} 0%,#034665 100%)`,WebkitBackgroundClip:'text',WebkitTextFillColor:'transparent',backgroundClip:'text' }}>{r.metric.value}</span>
        <span className="text-[0.70rem] md:text-[0.62rem] font-bold text-[#5a7a8a] tracking-[0.06em] uppercase">{r.metric.label}</span>
      </div>

      {/* divider */}
      <div className="h-px mb-5 md:mb-3" style={{ background:'linear-gradient(90deg,rgba(97,187,197,0.20),transparent)' }} />

      {/* footer */}
      <div className="flex items-center gap-3.5 md:gap-2.5">
        <div className="relative flex-shrink-0">
          <img src={r.image} alt={r.name} className="w-[52px] h-[52px] md:w-[40px] md:h-[40px] rounded-full object-cover object-top border-[3px] border-white block" style={{ boxShadow:'0 4px 14px rgba(3,70,101,0.14)' }} />
        ...
        <div className="flex-1 flex flex-col gap-0.5">
          <span className="text-[0.93rem] md:text-[0.82rem] font-extrabold text-[#0d3d5a]">{r.name}</span>
          <span className="text-[0.74rem] md:text-[0.66rem] text-[#7a9aaa] font-medium">{r.role}</span>
          <Stars count={r.rating} />
        </div>
        <span className="inline-flex items-center gap-[5px] px-3 py-[5px] md:px-2 md:py-[3px] rounded-full bg-[rgba(97,187,197,0.08)] border border-[rgba(97,187,197,0.22)] text-[0.66rem] md:text-[0.58rem] font-extrabold tracking-[0.05em] uppercase whitespace-nowrap flex-shrink-0 transition-colors duration-200 group-hover:bg-[rgba(97,187,197,0.14)]" style={{ color:r.color }}>
          <i className={`bi ${r.tagIcon} text-[0.72rem] md:text-[0.62rem]`} /> {r.tag}
        </span>
      </div>
    </div>
  );
...
export default function Testimonials() {
  ...
  return (
    <section className="relative bg-white py-14 md:py-16 font-['Inter','Nunito_Sans',sans-serif] overflow-visible isolate" ref={ref}>
      {/* bg glows */}
      ...
      <div className={`relative z-10 max-w-[1180px] mx-auto px-4 sm:px-6 md:px-12 transition-opacity duration-500 ${visible ? 'opacity-100' : 'opacity-0'}`}>
        ...
        <div className="flex items-center gap-5 mb-8" onMouseEnter={pause} onMouseLeave={resume}>
          ...
          <div key={animKey} className={`flex-1 grid gap-6 md:gap-4 min-w-0 ${isMobile?'grid-cols-1':'grid-cols-2'}`} style={{ animation:slideAnim }}>
        ...
        <div className="flex justify-center gap-2.5 mt-2">
          {Array.from({length:total}).map((_,i) => (
            <button key={i} aria-label={`Slide ${i+1}`}
              ...
              className={`w-2.5 h-2.5 rounded-full border-0 p-0 cursor-pointer transition-all duration-200 ${i===current?'':'bg-[rgba(97,187,197,0.25)]'}`}
