...
export default function Footer() {
  ...
  const linkCls = 'inline-flex items-center gap-1 text-[0.875rem] font-medium text-[#4a6a7a] no-underline transition-[color,transform] duration-200 hover:text-[#034665] hover:translate-x-1.5';
      ...
      <div ref={mid.ref} className="relative z-10 px-4 sm:px-8 md:px-12 py-10 sm:py-12 md:py-16 pb-8 sm:pb-10 md:pb-12">
        <div className={`max-w-[1280px] mx-auto grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-8 sm:gap-10 transition-opacity duration-500 ${mid.on ? 'opacity-100' : 'opacity-0'}`}>
          ...
          <div className={mid.on ? 'animate-[ftUp_0.7s_cubic-bezier(0.22,1,0.36,1)_both]' : ''}>
            ...
            <p className="text-[0.86rem] text-[#5a7a8a] leading-[1.8] mb-5 max-w-[280px]">
                ...
                const inner = (
                  <>
                    ...
                    <div className="flex flex-col gap-0.5">
                      {item.lines.map((l, j) => <span key={j} className="text-[0.82rem] text-[#4a6a7a] leading-[1.55] font-medium">{l}</span>)}
                    </div>
                  </>
                );
                return item.href ? (
